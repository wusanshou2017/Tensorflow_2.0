...
def predict_rnn(prefix, num_chars, rnn, params, init_rnn_state,
    ...
    for t in range(num_chars + len(prefix) - 1):
        ...
        X = tf.convert_to_tensor(
            to_onehot(np.array([output[-1]]), vocab_size), dtype=tf.float32)
        X = tf.reshape(X, [1, -1])
        # 计算输出和更新隐藏状态
...
def train_ch7(optimizer_fn, states, hyperparams, features, labels,
    ...
    w = tf.Variable(np.random.normal(0, 0.01, size=(
        features.shape[1], 1)), dtype=tf.float32)
...
def train_tensorflow2_ch7(trainer_name, trainer_hyperparams, features, labels,
    ...
    for _ in range(num_epochs):
        ...
        for batch_i, (X, y) in enumerate(data_iter):
            ...
            trainer.apply_gradients(
                zip(grads, net.trainable_variables))  # 迭代模型参数
            if (batch_i + 1) * batch_size % 100 == 0:
                ls.append(eval_loss())  # 每100个样本记录下当前训练误差
    # 打印结果和作图
    print('loss: %f, %f sec per epoch' % (ls[-1], time.time() - start))
